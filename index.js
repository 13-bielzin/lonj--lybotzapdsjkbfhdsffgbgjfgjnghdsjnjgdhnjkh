...
//DATAFILE
img1 = fs.readFileSync('./gambar/menuzz.jpg')
img2 = fs.readFileSync('./gambar/fake.jpg')
fakeyoi = ["MEGUMINARU bot"]
...
const wit = moment.tz('America/Sao_Paulo').format("HH:mm:ss")
...
teks += `* ala o cara perdeu o admin akakakak*🏃 :\n`
teks += `@_.split('@')[0]`
}
mentions(teks, mentioned, true)
Lexxy.groupDemoteAdmin(from, mentioned)
} else {
mentions(`Press F pelo(a) @${mentioned[0].split('@')[0]} *que perdeu o admin*🏃`, mentioned, true)
Lexxy.groupDemoteAdmin(from, mentioned)
}
break
case 'herolist':
...
teks = ''
for (let _ of mentioned) {
teks += `>_< oi amor, seja bem vindo(a) a administração >_< :\n`
teks += `@_.split('@')[0]`
}
mentions(teks, mentioned, true)
Lexxy.groupMakeAdmin(from, mentioned)
} else {
mentions(`oi amor @${mentioned[0].split('@')[0]} * seja muito bem vindo(a) a adiministração do grupo * >_<`, mentioned, true)
Lexxy.groupMakeAdmin(from, mentioned)
}
break
case 'bisakah':
...
break
case 'gitown':
reply2(`*GitHub do dono*\n${githubown}`)
break
case 'instaow':
reply2(`*instagram do dono*\n \n ⋯⋯⋯⋯⋯⋯⋯⋯⋯⋯ \n @ ➤ @lonly__kkkj \n ⋯⋯⋯⋯⋯⋯⋯⋯⋯⋯ \n link ➤ instgram.com/lonly__kkkj \n\n\n\n BY Meguminaru`)
break
case 'setfake':
if (!isOwner) return sticOwner(from)
...
Lexxy.sendMessage(from, res, image, {caption: `*${command}*`, quoted: flexx })
break
case 'estetikpic':
if (isBanned)return sticBanned(from)
sticWait(from)
...
for (let _ of anu) {
Lexxy.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 meguminaru  BOT 」*\n\n${body.slice(4)}`})
}
reply('LONLY Broadcast')
} else {
for (let _ of anu) {
sendMess(_.jid, `*「 meguminaru  BOT 」*\n\n${body.slice(4)}`)
}
...
ow =`*hello ${pushname}👋 quer descobrir mais sobre meu dono?*`
ow2 =`clique no botão abaixo para saber melhor sobre ele <3`
but = [
{ buttonId: `${prefix}gitown`, buttonText: { displayText: '𝐆𝐈𝐓𝐇𝐔𝐁' }, type: 1 },
{ buttonId: `${prefix}instaow`, buttonText: { displayText: '' }, type: 1 }
]
